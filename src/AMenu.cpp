...
AMenu::AMenu(const Json::Value& config, const std::string& name, const std::string& id,
...
auto AMenu::update() -> void { AModule::update(); }

std::string AMenu::getIcon(uint16_t percentage, const std::string& alt, uint16_t max) {
...
std::string AMenu::getIcon(uint16_t percentage, const std::vector<std::string>& alts,
                            uint16_t max) {
  auto format_icons = config_["format-icons"];
  if (format_icons.isObject()) {
...
bool waybar::AMenu::handleToggle(GdkEventButton* const& e) {
...
std::string AMenu::getState(uint8_t value, bool lesser) {
